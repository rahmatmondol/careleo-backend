...
import { enrichComments } from '../shared/enrich';
...
export const CommentsService = {
  ...
  async add(postId: string, userId: string, content: string, parentId?: string) {
    ...
    return { data: { comment: (await enrichComments([comment], userId))[0] }, status: 201 };
  },

  /** Top-level comments with their replies nested one level deep. */
  async list(postId: string, viewerId?: string | null) {
    ...
    // Authors are resolved for the whole thread in one pass — top-level rows
    // and replies together — rather than per comment.
    const enrichedTop = await enrichComments(top, viewerId);
    const enrichedReplies = await enrichComments(replies, viewerId);
    const repliesByParent = new Map<string, any[]>();
    for (const r of enrichedReplies) {
      const key = (r as any).parentId as string;
      if (!repliesByParent.has(key)) repliesByParent.set(key, []);
      repliesByParent.get(key)!.push(r);
    }

    const comments = enrichedTop.map((c) => ({ ...c, replies: repliesByParent.get(c.id) ?? [] }));
